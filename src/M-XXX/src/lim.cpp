...
{
	if ((LIM_CODE_LMD != _lim->nCode) && (LIM_CODE_LMD_RSSI != _lim->nCode) && (LIM_CODE_LMD_HDRSSI != _lim->nCode))
		return NULL;
	else
...
{
	if (LIM_CODE_LMD_HDRSSI != _lim->nCode)
		return NULL;
	else
	{
		return (LMD_HDRSSI_Type*)(LMD_Info(_lim) + 1);
	}
}
